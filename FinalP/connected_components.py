    ...
    for max_idx in max_vals:  # iterate through most valuable parcels
        ...
        if set_cost <= remaining_budget:
            ...
            for i in range(len(components[max_idx])):
                subgraph_to_v[i] = components[max_idx][i]
                v_to_subgraph[components[max_idx][i]] = i
            ...
            subgraph.node_weights = [[v_to_subgraph[graph.node_weights[subgraph_to_v[i]][0]], graph.node_weights[subgraph_to_v[i]][1]] for i in range(len(components[max_idx]))]
            ...
            for selected_node in sub_nodes:
                ...
                remaining_budget -= graph.node_weights[subgraph_to_v[selected_node]][0]
